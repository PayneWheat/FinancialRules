...
def test_strategy_constructor():
    rules = {"action": "disqualify", "parameters": ["person.state", "product.disqualified"], "values": ["Florida", True], "operations": ["==", "="]}
...
def test_strategy_comp_gt():
    rules = {"action": "decrease_rate", "parameters": ["person.credit_score", "product.interest_rate"], "values": [740, 0.5], "operations": [">", "="]}
    strat = Strategy(rules)
    assert strat.compOperation is operator.gt

def test_strategy_comp_ge():
    rules = {"action": "disqualify", "parameters": ["person.credit_score", "product.disqualified"], "values": [600, True], "operations": [">=", "="]}
    strat = Strategy(rules)
    assert strat.compOperation is operator.ge

def test_strategy_comp_lt():
    rules = {"action": "increase_rate", "parameters": ["person.credit_score", "product.interest_rate"], "values": [650, 1.5], "operations": ["<", "="]}
    strat = Strategy(rules)
